...
it('applies lower case transformation', function (): void {
    config(['slug.case' => 'lower']);
    $model = TestModel::create(['name' => 'Hello World']);
    expect($model->slug)->toBe('hello-world');
});

it('applies title case transformation', function (): void {
    config(['slug.case' => 'title']);
    $model = TestModel::create(['name' => 'Hello World']);
    expect($model->slug)->toBe('Hello World');
});

it('applies camel case transformation', function (): void {
    config(['slug.case' => 'camel']);
    $model = TestModel::create(['name' => 'Hello World']);
    expect($model->slug)->toBe('helloWorld');
});

it('applies snake case transformation', function (): void {
    // Str::snake() only converts camelCase boundaries to underscores; by this
    // point the slug has already gone through Str::slug(), which lowercases
    // and hyphenates everything, so there's no camelCase boundary left for
    // Str::snake() to act on. The 'snake' case option is a no-op in practice
    // for slugs generated by this package — this test documents that, not an
    // expectation that it rewrites hyphens to underscores.
    config(['slug.case' => 'snake']);
    $model = TestModel::create(['name' => 'Hello World']);
    expect($model->slug)->toBe('hello-world');
});

it('leaves the slug untouched for an unrecognized case option', function (): void {
    config(['slug.case' => 'not-a-real-option']);
    $model = TestModel::create(['name' => 'Hello World']);

    expect($model->slug)->toBe('hello-world');
});

it('uses a custom suffix separator for duplicate slugs', function (): void {
    config(['slug.suffix_separator' => '_']);

    TestModel::create(['name' => 'Hello World']);
    $second = TestModel::create(['name' => 'Hello World']);

    expect($second->slug)->toBe('hello-world_1');
});
